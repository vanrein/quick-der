...
class ASN1ConstructedType (ASN1Object):
	...
	def __init__ (self, bindata=[], ofs=0, structure=None):
		...
		for (k,v) in structure.items ():
			...
			newval = build_asn1 (v, bindata, ofs)
			if isinstance (newval, ASN1Object):
				# Special: ASN.1 data returned; set in _bindata
				self._bindata [ofs] = newval
			else:
				# Standard: Index into _bindata; set in _fields
				self._fields [k] = newval
...
class ASN1Atom (ASN1Object):
	...
	def __init__ (self, bindata=[''], ofs=0):
		self.bindata = bindata
		self.offset = ofs

	def get (self):
		return self.bindata [self.offset]

	def set (self, value):
		tv = type (value)
		if tv == str:
			self.bindata [self.offset] = value
		elif tv == int:
			strval = ''
			...
			self.bindata [self.offset] = strval
		else:
...
def build_asn1 (typdesc, bindata=[], ofs=0):
	"""Construct an ASN.1 structural element from a type description and
	   bindata.  The result can either be an ASN1Object subclass instance
	   or an offset into bindata.
	"""
	t = type (typdesc)
	if t == int:
		...
		return ASN1ConstructedType (bindata, ofs, structure=typdesc)
	else:
		raise ValueError ("ASN.1 type description must be int, dict, list, set or (subclass,suboffset)")
